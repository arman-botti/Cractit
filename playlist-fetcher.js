...
// Save to DB
function saveVideos(videos, category, label) {
  const insert = db.prepare(`
    INSERT OR IGNORE INTO videos (title, description, youtube_id, category, label, thumbnail, duration, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  const insertMany = db.transaction((vids) => {
    for (const v of vids) {
      const r = insert.run(v.title, v.description || '', v.videoId, category, label || category, v.thumbnail, '0:00', v.published);
...
async function fetchPlaylist(playlistId, category, label) {
  ...
  try {
    ...
    const added = saveVideos(videos, category, label);
