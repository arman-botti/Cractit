...
function genAdminSession() {
  ...
  setTimeout(() => adminSessions.delete(token), 8*60*60*1000); // 8hr expire
  return token;
}
...
<head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .err{color:#f43f5e;font-size:13px;margin-top:12px;display:none}
  </style>
</head>
<body>
  <div class="box">
    ...
    <input type="password" id="pw" placeholder="Admin password" autofocus onkeydown="if(event.key==='Enter')login()">
    <button onclick="login()">Login →</button>
    <div class="err" id="err">❌ Wrong password!</div>
  </div>
  <script>
    // Check saved session
    const saved = sessionStorage.getItem('crackit_adm');
    if(saved) {
      fetch('/admin-check', {headers:{'x-admin-token':saved}})
        .then(r => { if(r.ok) loadAdmin(); });
    }
    ...
    async function login() {
      ...
      try {
        ...
        if(r.ok) {
          sessionStorage.setItem('crackit_adm', data.token);
          loadAdmin();
        } else {
    ...
    function loadAdmin() {
      const token = sessionStorage.getItem('crackit_adm');
      fetch('/admin-file', {headers:{'x-admin-token':token}})
        .then(r => r.text())
...
// Serve actual admin file after auth check
