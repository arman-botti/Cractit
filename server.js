...
const adminSessions = new Set(); // In-memory session store

// Generate session token
function genAdminSession() {
...
<head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .box{background:#0d1120;border:1px solid rgba(255,255,255,0.1);border-radius:20px;padding:40px;width:340px;text-align:center;box-shadow:0 25px 50px rgba(0,0,0,0.5)}
    .icon{font-size:48px;margin-bottom:16px}
    h2{color:#fff;font-size:24px;font-weight:800;margin-bottom:6px}
    p{color:#6b7280;font-size:13px;margin-bottom:28px}
    input{width:100%;background:#111827;border:1.5px solid rgba(255,255,255,0.1);color:#fff;padding:13px 16px;border-radius:10px;font-size:15px;outline:none;margin-bottom:16px;transition:border .2s}
    input:focus{border-color:#4f46e5}
    button{width:100%;background:linear-gradient(135deg,#4f46e5,#7c3aed);color:#fff;border:none;padding:13px;border-radius:10px;cursor:pointer;font-size:15px;font-weight:700;transition:opacity .2s}
    button:hover{opacity:0.9}
    .err{color:#f43f5e;font-size:13px;margin-bottom:14px;background:rgba(244,63,94,0.1);padding:10px;border-radius:8px;${error ? '' : 'display:none'}}
  </style>
</head>
<body>
  <div class="box">
...
function adminAuth(req, res, next) {
  const sessionToken = req.cookies && req.cookies['crackit_admin'];
  if(sessionToken && adminSessions.has(sessionToken)) return next();
...
app.post('/admin-auth', express.json(), (req, res) => {
  ...
  if(password === ADMIN_SECRET) {
    const token = genAdminSession();
    res.cookie('crackit_admin', token, {
      httpOnly: true,
...
app.get('/admin-logout', (req, res) => {
  const token = req.cookies && req.cookies['crackit_admin'];
  if(token) adminSessions.delete(token);
  res.clearCookie('crackit_admin');
  res.redirect('/manage-crackit-x9z2026');
});


// Protect admin pages
// Secret admin URL (hide from public)
...
// Old /admin.html redirect to home
app.get('/admin.html', (req, res) => { res.redirect('/'); });
app.get('/DISABLED-admin', adminAuth, (req, res) => {
  const fs = require('fs');
  const adminPath = require('path').join(__dirname, 'admin.html');
  const adminPath2 = require('path').join(__dirname, 'public', 'admin.html');
  const file = fs.existsSync(adminPath) ? adminPath : adminPath2;
