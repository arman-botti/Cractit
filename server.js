...
<head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    input{width:100%;background:#111827;border:1.5px solid rgba(255,255,255,0.1);color:#fff;padding:13px 16px;border-radius:10px;font-size:15px;outline:none;margin-bottom:16px}
    ...
    .err{color:#f43f5e;font-size:13px;margin-top:12px;${error ? '' : 'display:none'}}
  </style>
</head>
<body>
  <div class="box">
    ...
    <form method="POST" action="/manage-crackit-x9z2026">
      <input type="password" name="adminpw" placeholder="Admin password" autofocus>
...
function adminAuth(req, res, next) {
  // Check POST body password
  const bodyPw = req.body && req.body.adminpw;
  // Check cookie
  const cookiePw = req.cookies && req.cookies['cadm'];
  
  if(bodyPw === ADMIN_SECRET || cookiePw === ADMIN_SECRET) {
    return next();
  }
  
  const error = (req.method === 'POST' && bodyPw) ? '❌ Wrong password! Try: Ruchika0830' : '';
  res.send(adminLoginPage(error));
}

// Protect admin pages
// Secret admin URL
app.all('/manage-crackit-x9z2026', adminAuth, (req, res) => {
  // Set session cookie on successful login
  if(req.method === 'POST') {
    res.cookie('cadm', ADMIN_SECRET, { maxAge: 28800000, httpOnly: false, path: '/' });
  }
  const fs = require('fs');
